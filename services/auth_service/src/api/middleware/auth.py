...
class AuthMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        if request.url.path in ["/health", "/auth/login", "/auth/register"]:
            return await call_next(request)  # Skip authentication for health and auth endpoints
        ...
        try:
            user = await self.user_manager.get_user_by_token(credentials.credentials)
            if not user:
...
def admin_required(required_roles: Optional[List[str]] = None):
    ...
    async def check_roles(request: Request):
        ...
        if not user:
            logging_client.warning(f"User not authenticated")
