...
class BotRepository:
    ...
    async def create(self, **bot_data) -> Bot:
        ...
        try:
            ...
            logger.info(f"Bot created successfully with id: {bot.id}")
            return bot
    ...
    async def get(self, bot_id: int, user_id: int) -> Optional[Bot]:
        ...
        try:
            ...
            raise DatabaseException(f"Failed to get bot with id {bot_id}: {e}") from e

    @handle_exceptions
    async def get_all_by_user_id(self, user_id: int) -> List[Bot]:
        """
        Retrieves all bots for a specific user ID.
        """
    ...
    @handle_exceptions
    async def update(self, bot_id: int, user_id: int, **bot_data) -> Optional[Bot]:
        """
        Updates an existing bot by its ID.
        """
        logger.info(f"Updating bot with ID: {bot_id}, user_id: {user_id}, data: {bot_data}")
        try:
           query = select(Bot).where(Bot.id == bot_id, Bot.user_id == user_id)
           result = await self.session.execute(query)
           bot = result.scalar_one_or_none()
           ...
           return bot
            ...
            raise DatabaseException(f"Failed to update bot with id {bot_id}: {e}") from e

    @handle_exceptions
    async def delete(self, bot_id: int, user_id: int) -> Optional[Bot]:
        """
        Deletes a bot by its ID and user_id.
        """
        logger.info(f"Deleting bot with ID: {bot_id} for user_id: {user_id}")
        try:
            query = delete(Bot).where(Bot.id == bot_id, Bot.user_id == user_id)
            result = await self.session.execute(query)
            await self.session.commit()
            logger.info(f"Bot with ID {bot_id} deleted successfully")
            return result.scalar_one_or_none()
        except Exception as e:
             logger.error(f"Error deleting bot with ID {bot_id}: {e}")
             raise DatabaseException(f"Failed to delete bot with id {bot_id}: {e}") from e