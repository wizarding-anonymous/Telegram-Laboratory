...
class BlockRepository:
    ...
    @handle_exceptions
    async def create(self, block_data: Dict[str, Any]) -> Block:
        """Creates a new block."""
    ...
    @handle_exceptions
    async def list_paginated(self, page: int, page_size: int, bot_id: int) -> Tuple[List[Block], int]:
        ...
        count_query = await self.session.execute(select(func.count()).where(Block.bot_id == bot_id))
