...
class StateMachineHandler:
    ...
    @handle_exceptions
    async def handle_state_machine(self, block: Dict[str, Any], chat_id: int, variables: Dict[str,Any], current_block_id: int) -> Optional[int]:
        """
        ...
        for transition in transitions:
            ...
            if condition_type == "message_text":
                ...
                if last_user_message == condition_value:
                 ...
                 if variables.get(condition_value) == transition.get("variable_value"):
                    await redis_client.set(user_key, to_state)
                    return next_block_id
    ...
    @handle_exceptions
    async def save_user_message(self, message: Dict[str, Any], chat_id: int):
