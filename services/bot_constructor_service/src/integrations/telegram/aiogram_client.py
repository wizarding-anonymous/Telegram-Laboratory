...
class AiogramClient(TelegramClient):
    """
    Client for interacting with the Telegram Bot API using aiogram.
    """
    ...
    async def check_connection(self, bot_token: str = None) -> bool:
        ...
        try:
            ...
            if bot_info:
                logger.info(f"Aiogram token is valid for user: {bot_info.username}")
                return True
            else:
                logger.error("Aiogram token is invalid")
                return False
    ...
    async def send_message(self, chat_id: int, text: str, parse_mode: str = "HTML", reply_markup: Optional[Any] = None, inline_keyboard: Optional[Any] = None) -> dict:
        ...
        try:
            if reply_markup:
                 keyboard = ReplyKeyboardMarkup(keyboard=reply_markup, resize_keyboard=True, one_time_keyboard=True)
    ...
    async def send_venue(self, chat_id: int, latitude: float, longitude: float, title: str, address: str) -> dict:
        """
        Sends a venue to a Telegram chat using aiogram.
        Args:
            chat_id (int): Chat ID to send the location to.
    ...
    async def send_game(self, chat_id: int, game_short_name: str) -> dict:
        """
        Sends a game to a Telegram chat using aiogram.
        Args:
    ...
    async def send_poll(self, chat_id: int, question: str, options: List[str]) -> dict:
        """
        Sends a poll to a Telegram chat using aiogram.
        Args:
